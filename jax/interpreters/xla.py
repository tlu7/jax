...
def _prune_unused_inputs(
  ...
  if lib._xla_extension_version < 18:
    return jaxpr, set(jaxpr.constvars), set(jaxpr.invars)
  used = {v for v in jaxpr.outvars if isinstance(v, core.Var)}
  # Use a conservative pruning algorithm.
  # Infeed and host callback are side-effecting primitives and cannot be pruned.
  used.update(v for eqn in jaxpr.eqns for v in eqn.invars if isinstance(v, core.Var))
